    ...
    public PredictionResult predictCatBreed(MultipartFile image) throws IOException {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                log.error("预测服务调用失败，HTTP状态码: {}", response.getStatusCode());
                throw new BusinessException(CatcatEnumClass.StatusCode.INTERNAL_SERVER_ERROR.getCode(), CatcatEnumClass.StatusCode.INTERNAL_SERVER_ERROR.getMessage());
            }
        } catch (Exception e) {
            log.error("预测服务调用失败", e);
            throw new BusinessException(CatcatEnumClass.StatusCode.INTERNAL_SERVER_ERROR.getCode(), CatcatEnumClass.StatusCode.INTERNAL_SERVER_ERROR.getMessage());
        }
    }
} 